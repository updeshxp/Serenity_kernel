...
/*---------------------------------------------------------------------------
    wpalPacketRXLowResourceCB \96 RX RAW packer CB function
    Param: 
        pPacket \96 Available RX packet
...
/*---------------------------------------------------------------------------
    ...
    Param: 
        pPkt \96 pointer to a wpt_packet
    Return:
...
/*---------------------------------------------------------------------------
    wpalPacketGetLength \96 Get number of bytes in a wpt_packet. It include the 
...
/*---------------------------------------------------------------------------
    wpalPacketRawTrimHead \96 Move the starting offset and return the head pointer
...
/*---------------------------------------------------------------------------
    wpalPacketRawTrimTail \96 reduce the length of the packet.
    Param: 
        pPkt - pointer to a wpt_packet.
...
/*---------------------------------------------------------------------------
    wpalPacketGetRawBuf \96 Return the starting buffer virtual address for the RAW flat buffer
...
/*---------------------------------------------------------------------------
    ...
    Param: 
        pIter \96 pointer to a caller allocated wpt_iterator
        pPacket \96 pointer to a wpt_packet
        ppAddr \96 Caller allocated pointer to return the address of the item.
...
/*---------------------------------------------------------------------------
    wpalLockPacketForTransfer \96 Map the data buffer from dma so that the
...
/*---------------------------------------------------------------------------
    wpalUnlockPacket \96 Unmap the data buffer from dma so that cpu can regain
                          ownership on it
    Param: 
        pPacket \96 pointer to a wpt_packet
