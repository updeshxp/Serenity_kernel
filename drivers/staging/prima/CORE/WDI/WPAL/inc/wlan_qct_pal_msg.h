...
/*---------------------------------------------------------------------------
   wpalPostCtrlMsg \96 Post a message to control context so it can 
                           be processed in that context.
   Param: 
      pPalContext \96 A PAL context
      pMsg \96 a pointer to called allocated object; Caller retain the ownership 
...
/*---------------------------------------------------------------------------
   wpalPostTxMsg \96 Post a message to TX context so it can be processed in that context.
   Param: 
      pPalContext \96 A PAL context
      pMsg \96 a pointer to called allocated object; Caller retain the ownership 
             after this API returns.
...
/*---------------------------------------------------------------------------
   wpalPostRxMsg \96 Post a message to RX context so it can be processed in that context.
   Param: 
      pPalContext \96 A PAL context
      pMsg \96 a pointer to called allocated object; Caller retain the ownership 
             after this API returns.
